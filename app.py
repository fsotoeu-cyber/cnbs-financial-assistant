...
def filtrar_instituciones(df, query=None, bancos=None):
    ...
    if uni == "estatales":
        ...
        return sub if not sub.empty else d
    # comerciales (default) o sistema → comerciales para rankings de entidades
    if d["TipoInstitucion"].nunique() > 1:
        solo = d[d["TipoInstitucion"].astype(str).str.upper().str.contains("BANCOS COMERCIALES", na=False)]
        if not solo.empty:
            return solo
    return d
...
def respuesta_centro_ayuda():
    ...
    return """### 👋 Centro de ayuda — Agente CNBS
...
def es_consulta_equilibrio_triple(query):
    ...
    explicito_score = any(p in q for p in (
        "score de equilibrio", "score triple", "score (roe", "ranking por score",
...
def formatear_sistema_corto(df_res, anios=None):
    ...
    titulo = "Sistema bancario hondureño (promedio de bancos)"
    if anios:
    ...
    if "Indicador" in df_res.columns:
        ...
        if notas:
            ...
            lineas.append("")
        lineas.append(
            f"_Indicadores: {df_res['Indicador'].nunique()} · "
            f"Valores = promedio entre bancos (no es un banco llamado «Sistema»)._"
        )
...
def aviso_universo_pequeno(df_res):
    """Credibilidad: si el filtro deja < 3 instituciones, avisar."""
    n = contar_entidades_resultado(df_res)
    if n <= 0:
        return ""
    if n == 1:
        nombre = str(df_res["Banco"].iloc[0]) if "Banco" in df_res.columns else ""
        extra = f" (**{nombre}**)" if nombre else ""
        return (
            "\n\n> ⚠️ **Universo reducido:** el dataset solo contiene **1 institución**"
            f"{extra} para este filtro. El resultado es **descriptivo** y no representa "
            "un ranking competitivo."
        )
    if n < 3:
        return (
            f"\n\n> ⚠️ **Universo reducido:** solo **{n} instituciones** en este filtro. "
            "Interpreta el resultado con cautela."
        )
    return ""


def hallazgos_automaticos_pandas(df_res):
    """Cierres determinísticos 100% Pandas (sin LLM). Máximo 6 viñetas."""
    if df_res is None or df_res.empty:
        return []
    h = []
    try:
        if "Score_Triple" in df_res.columns and "Banco" in df_res.columns:
            ...
            if "Morosidad" in df_res.columns and df_res["Morosidad"].notna().any():
                m = df_res.loc[df_res["Morosidad"].idxmin()]
                h.append(f"Menor morosidad en el ranking: **{m['Banco']}** ({float(m['Morosidad']):.2f}%)")
        ...
        if "Ratio_ROE_Mora" in df_res.columns and "Banco" in df_res.columns:
            ...
            if "Morosidad" in df_res.columns and df_res["Morosidad"].notna().any():
                m = df_res.loc[df_res["Morosidad"].idxmin()]
                h.append(f"Menor mora: **{m['Banco']}** ({float(m['Morosidad']):.2f}%)")
            return h[:6]
...
def bloque_hallazgos(df_res):
    """Markdown: aviso universo pequeño + hallazgos Pandas."""
    partes = []
    av = aviso_universo_pequeno(df_res)
    if av:
        partes.append(av)
    halls = hallazgos_automaticos_pandas(df_res)
    if halls:
...
def responder_directamente(df_res, query, meta_info):
    ...
    if "Ranking" in df_res.columns and "Score_Triple" in df_res.columns:
        return formatear_tabla_ranking_triple(df_res) + bloque_hallazgos(df_res)
    if "Ranking" in df_res.columns and "Ratio_ROE_Mora" in df_res.columns:
        return formatear_tabla_ranking(df_res) + bloque_hallazgos(df_res)

    if consulta_simple_sistema(query, df_res):
        return formatear_sistema_corto(df_res, meta_info.get("anios")) + bloque_hallazgos(df_res)
    ...
    if "Indicador" in df_res.columns and (
        ...
        if tab:
            return tab + bloque_hallazgos(df_res)
    ...
    if "Indicador" in df_res.columns and df_res["Indicador"].nunique() == 1:
        ...
        return "\n".join(lineas) + bloque_hallazgos(df_res)


    base = df_a_markdown(df_res) if hasattr(df_res, "iterrows") else str(df_res)
    return base + bloque_hallazgos(df_res)
...
# Universo por defecto: instituciones no agregadas; si hay TipoInstitucion → solo bancos comerciales
_df_inst = df[~df["Banco"].map(es_agregado)].copy()
...
n_bancos = int(_df_inst["Banco"].nunique())
...
if "TipoInstitucion" in df.columns:
    tipos_institucion_lista = sorted(
        t for t in df["TipoInstitucion"].dropna().astype(str).unique().tolist()
        if t.strip() and not es_agregado(t) and t.upper() not in {"FINANCIERAS", "BANCOS ESTATALES", "BANCOS"}
    )
...
_Fuente: dataset CNBS del agente. Escribe el tipo en la pregunta si no quieres solo comerciales._
        """)
    ...
    if not groq_api_key:
        ...
        if not st.session_state.messages:
            ...
            <div class="welcome-box">
                <div style="font-size:18px;font-weight:600;margin-bottom:6px;">👋 Bienvenido al Agente Financiero CNBS</div>
                <div style="font-size:14px;line-height:1.55;">
                    Analizo <b>indicadores oficiales</b> de la CNBS (ROA, ROE, morosidad, capital, liquidez…).<br/>
                    Los cálculos los hace <b>Pandas</b> — no invento cifras.<br/><br/>
    ...
    with f2:
        ...
        if "TipoInstitucion" in df.columns and tipos_institucion_lista:
            ...
            if tipo_sel == "Todos los tipos":
                df_tipo = base_t
                label_ms = "Instituciones (todos los tipos)"
            else:
                df_tipo = base_t[base_t["TipoInstitucion"].astype(str) == tipo_sel]
                label_ms = "Bancos" if "COMERCIALES" in str(tipo_sel).upper() else "Instituciones"
        ...
        sel_all = st.checkbox(f"Seleccionar todos ({label_ms.lower()})", value=False, key="tend_all")
        if sel_all:
            ...
            bancos_sel = st.multiselect(
                label_ms,
                lista_inst,
                default=lista_inst[:5] if len(lista_inst) >= 5 else lista_inst,
                key="tend_bancos",
            )
...
with tab3:
    ...
    # Opciones desde el dataset filtrado
