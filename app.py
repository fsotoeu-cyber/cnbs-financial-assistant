...
def formatear_sistema_corto(df_res, anios=None):
    ...
    titulo = "Promedio del sistema bancario (agregado estadístico, no una institución)"
    if anios:
...
def aviso_universo_pequeno(df_res):
    ...
    if es_sistema:
        # Texto según el tipo de agregado
        if "FINANCIER" in nu:
            quien = "promedio del grupo de **sociedades financieras**"
        elif "ESTATAL" in nu:
            quien = "promedio del grupo de **bancos estatales**"
        elif "COMERCIAL" in nu:
            quien = "promedio del grupo de **bancos comerciales**"
        else:
            quien = "promedio del **sistema bancario** (agregado estadístico)"
        return (
            chr(10) + chr(10)
            + f"> ℹ️ **Agregado estadístico:** {quien}. "
            "No es una institución individual ni un ranking. "
            "El **Score de equilibrio** se calcula solo entre bancos o financieras uno a uno."
        )
    if n == 1:
...
def hallazgos_automaticos_pandas(df_res):
    ...
    try:
        ...
        # Una sola entidad o agregado → "Resumen", nunca "Mayor X"
        if n <= 1 or (es_sistema and n <= 3):
            if "Indicador" in df_res.columns and "Saldo" in df_res.columns:
                for _, row in df_res.iterrows():
                    ...
                    if "Año" in row.index and pd.notna(row.get("Año")):
                        anio = f" ({int(row['Año'])})"
                    # Etiqueta tipo promedio cuando es agregado o una sola fila
                    if es_sistema or n <= 1:
...
def responder_directamente(df_res, query, meta_info):
    ...
    # Ranking de UN indicador: tabla ordenada con # (prioridad sobre comparación)
    if (
        "Indicador" in df_res.columns
        ...
        for _, row in orden.iterrows():
            rk = int(row["Ranking"]) if pd.notna(row.get("Ranking")) else 0
