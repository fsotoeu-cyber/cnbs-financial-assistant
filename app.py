...
def indicadores_pedidos_no_disponibles(query, catalogo):
    """Indicadores explícitos pedidos por el usuario que no existen en el catálogo CNBS."""
    q = normalizar_texto(query)
    pedidos = []
    # Eficiencia operativa (no publicada en el dump actual de ratios CNBS usado por la app)
    frases_eficiencia = (
        "eficiencia",
    ...
    if any(f in q for f in frases_eficiencia):
        encontrado = False
        valores = [normalizar_texto(str(v)) for v in catalogo.values()]
        claves = [normalizar_texto(str(k)) for k in catalogo.keys()]
        for texto in valores + claves:
            if "eficiencia" in texto:
                encontrado = True
                break
            if "gasto" in texto and "administr" in texto:
                encontrado = True
                break
            if "gastos" in texto and "ingreso" in texto:
                encontrado = True
                break
        if not encontrado:
            pedidos.append(
