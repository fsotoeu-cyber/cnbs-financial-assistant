...
def filtrar_instituciones(df, query=None):
    """
    Excluye agregados. Si el CSV trae TipoInstitucion y la consulta
    menciona financieras/estatales, filtra ese universo; si no, por
    defecto solo bancos comerciales cuando exista la columna.
    """
    d = df.copy()
    ...
    if any(p in q for p in ("financiera", "financieras", "sociedad financiera", "leasing")):
        return d[d["TipoInstitucion"].astype(str).str.upper().str.contains("FINANCIER", na=False)]
    if any(p in q for p in ("estatal", "estatales", "banhprovi")):
        return d[d["TipoInstitucion"].astype(str).str.upper().str.contains("ESTATAL", na=False)]
    # Default: bancos comerciales si hay tipos mezclados
...
2. Para ordenar **solo por capital**, no mezcles ROE ni morosidad en la misma frase.
3. Usa los **botones** de consultas sugeridas si no sabes por dónde empezar.

_Glosario de indicadores: expande **¿Qué significan los indicadores?** en la barra lateral._
"""
...
def planificar(query, contexto, catalogo):
    ...
    if any(p in q for p in (
        ...
        if inds:
            # mayor liquidez = descendente; "mejor capacidad" = mayor liquidez
...
_Fuente: definiciones operativas sobre el dataset CNBS del agente._
        """)
        ...
        if not st.session_state.messages:
            ...
            <div class="welcome-box">
                ...
                <div style="font-size:14px;line-height:1.55;">
                    ...
                    <b>Puedes:</b> consultar bancos · comparar indicadores · ver evolución · descargar PDF<br/><br/>
                    💡 Prueba un <b>botón</b> abajo o escribe <b>¿qué puedo preguntar?</b>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
with tab2:
    ...
    with f2:
        ...
        if "TipoInstitucion" in df.columns and tipos_institucion_lista:
            tipo_opts = tipos_institucion_lista
            # Preferir bancos comerciales como default
            def_i = 0
            for i, t in enumerate(tipo_opts):
                if "COMERCIALES" in t.upper():
                    def_i = i
                    break
            tipo_sel = st.selectbox(
                ...
                index=def_i,
                key="tend_tipo_inst",
                help="No mezclar bancos con financieras u otros tipos en el mismo gráfico.",
            )
            df_tipo = df[
                (df["TipoInstitucion"].astype(str) == tipo_sel)
                & (~df["Banco"].map(es_agregado))
            ]
            lista_inst = sorted(df_tipo["Banco"].dropna().unique().tolist())
            label_ms = "Instituciones" if "COMERCIALES" not in tipo_sel.upper() else "Bancos"
        else:
            tipo_sel = None
