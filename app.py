...
def indicadores_pedidos_no_disponibles(query, catalogo):
    """Frases de indicador explícitas que el usuario pidió y no están en el catálogo."""
    q = normalizar_texto(query)
    # Solo avisamos si la consulta nombra el concepto y no hay match en catálogo
    pedidos = []
    reglas = [
        (
            ["gastos de administracion", "gastos de administración", "gastos / ingresos",
             "gastos/ingresos", "eficiencia operativa", "indice de eficiencia",
             "nivel de eficiencia", "eficiencia (gastos"],
            ["eficiencia", "gastos de administracion", "gastos administracion"],
        ),
    ]
    for frases, claves_busqueda in reglas:
        if not any(f in q for f in frases) and "eficiencia" not in q:
            continue
        if "eficiencia" in q or any(f in q for f in frases):
            encontrado = False
            for c in claves_busqueda + ["eficiencia"]:
                if buscar_indicador(c, catalogo):
                    encontrado = True
                    break
            # también buscar por substring en valores del catálogo
            if not encontrado:
                for v in catalogo.values():
                    vu = normalizar_texto(str(v))
                    if "eficiencia" in vu or ("gasto" in vu and "administr" in vu):
                        encontrado = True
                        break
            if not encontrado:
                pedidos.append("eficiencia / Gastos de Administración sobre Ingresos Totales")
    return pedidos
                ...
                with st.spinner("Procesando con motor Pandas..."):
                    ...
                    try:
                        ...
                        if es_consulta_conversacional(query):
                            ...
                            _solo_faltante = bool(faltan) and not any(
                                p in normalizar_texto(query)
                                for p in (
                                    "roa", "roe", "morosidad", "adecuacion", "capital",
                                    "liquidez", "spread", "cobertura", "tarjeta",
                                )
                            )
                            if _solo_faltante:
                                output = (
                                    "⚠️ **Indicador no disponible en el dataset CNBS cargado:** "
                                    + "; ".join(faltan)
                                    + "."
                                    + chr(10) + chr(10)
                                    + "No es posible calcular esa métrica ni compararla. "
                                    "Prueba con ROA, ROE, morosidad, adecuación de capital, "
                                    "liquidez, cobertura o spread."
                                )
                        ...
                        if fuente == "Conversacional":
                            ...
                            # Nota si se pidió un indicador inexistente junto a otros
                            try:
                                if faltan and output:
                                    output = (
                                        output
                                        + chr(10) + chr(10)
                                        + "> ℹ️ **Nota:** el indicador *"
                                        + "; ".join(faltan)
                                        + "* no está en el dataset CNBS; esa parte de la consulta no se calculó."
                                    )
                            except Exception:
                                pass
