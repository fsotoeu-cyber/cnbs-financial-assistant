...
def filtrar_instituciones(df, query=None, bancos=None):
    ...
    # Instituciones nombradas: no forzar un solo tipo (permite cruzar)
    if bancos:
        return d

    cruce = any(p in q for p in (
        "todos los tipos", "todas las instituciones", "cruzar",
        "bancos y financieras", "financieras y bancos",
        "comerciales y estatales", "todo el sistema regulado",
        "entre tipos", "varios tipos", "mixto",
    ))
    if cruce:
        return d
    ...
    # Default: solo bancos comerciales si hay más de un tipo
