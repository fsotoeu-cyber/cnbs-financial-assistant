...
def detectar_universo(query, bancos=None):
    ...
    comerciales | estatales | financieras | todos | sistema | comerciales (default)
    """
    q = normalizar_texto(query or "")
    if bancos:
        return "especifico"
    ...
    )) and not any(p in q for p in ("comercial", "comerciales", "estatal", "estatales")):
        return "financieras"
    if any(p in q for p in ("estatal", "estatales", "banhprovi")) and "comercial" not in q:
        return "estatales"
                ...
                with st.spinner("Procesando con motor Pandas..."):
                    ...
                    try:
                        ...
                        meta_html = (
                            ...
                            f'📌 Motor: <b>{motor}</b> · 🏛️ Universo: <b>{_uni_lbl}</b><br>'
