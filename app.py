...
def aviso_universo_pequeno(df_res):
    """Credibilidad: avisar solo si hay 1 institución o etiqueta de sistema/agregado."""
    ...
    if es_sistema:
        return (
            chr(10) + chr(10) + "> ℹ️ **Promedio agregado:** esta fila resume indicadores del grupo/sistema, "
            "no es un ranking de instituciones individuales. "
            "El **Score de equilibrio** está pensado para comparar bancos o financieras uno a uno."
        )
    if n == 1:
...
def hallazgos_automaticos_pandas(df_res):
    ...
    try:
        ...
        # Una sola fila / promedio sistema → no inventar "Mayor ROE: Sistema"
        if n <= 1 or (es_sistema and n <= 2):
            if "Indicador" in df_res.columns and "Saldo" in df_res.columns:
                for _, row in df_res.iterrows():
                    et = etiqueta_corta_indicador(row["Indicador"])
                    banco = row.get("Banco", "")
                    h.append(f"{et}: **{float(row['Saldo']):.2f}%**")
                if es_sistema:
                    h.insert(0, "Valores = promedio del grupo (no ranking de Score)")
                return h[:8]
