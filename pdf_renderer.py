"""
pdf_renderer.py — Renderer universal de informes CNBS (v6.3).
...
def _conclusion_desde_df(df: Optional[pd.DataFrame]) -> Optional[str]:
    """Conclusión automática desde la fila #1 (determinística)."""
...
def _extraer_resumen_desde_texto(respuesta: str, dataframe: Optional[pd.DataFrame]) -> Optional[str]:
    if dataframe is not None and not dataframe.empty:
        ...
        if "banco" in cols and any("mora" in c for c in cols):
            ...
            orden = dataframe.sort_values(mora_col, ascending=False)
            top = orden.iloc[0]
            try:
                ...
                v = str(top[mora_col])
            return (
                f"La mayor morosidad corresponde a {top[cols['banco']]} ({v}), "
                f"según los datos CNBS del periodo analizado."
            )
    limpio = _limpiar_md(respuesta)
...
class PDFRenderer:
    ...
    def render(
        ...
        if not summary:
            summary = _extraer_resumen_desde_texto(respuesta, dataframe)

        respuesta_limpia = _limpiar_md(respuesta)
        # Conservar prosa del LLM; solo quitar filas de ranking crudo si ya hay tabla
        if dataframe is not None and not getattr(dataframe, "empty", True):
            ...
            for ln in respuesta_limpia.split("\n"):
                ...
                if re.match(r"^\d+\s+[A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ0-9 .\-]{1,40}\s+[\d.\-]+", s):
                    continue
                if re.match(r"^(Ranking\s+Banco|Banco\s+ROE)", s, re.I):
                    continue
                lineas.append(s)
            tmp, prev = [], False
            for ln in lineas:
    ...
    def _add_page_number(self, canvas, doc) -> None:
        ...
        text = (
            f"Agente Financiero CNBS {self.VERSION}  ·  "
    ...
    def _header(
        ...
        story.append(
            self.Paragraph(
                "<font size='11' color='#475569'>Agente Financiero CNBS</font>",
    ...
    def _footer_block(self, story, metadata=None) -> None:
        ...
        story.append(
            self.Paragraph(
                ...
                f"Generado automáticamente por Agente Financiero CNBS {self.VERSION}"
                "</font>",
                self.styles["Italic"],
            )
        )
    ...
    def _render_stdlib(
        ...
        if dataframe is not None and not getattr(dataframe, "empty", True):
            ...
            lines.append("")
        lines.append(
            f"Agente Financiero CNBS {self.VERSION} | Fuente: CNBS Honduras"
        )
